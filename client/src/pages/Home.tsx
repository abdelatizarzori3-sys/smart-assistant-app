...
export default function Home() {
  ...
  const conversationStarted = chatMessages.length > 0 || Boolean(voiceDraft) || attachedFiles.length > 0 || pendingFiles.length > 0;
  ...
  return (
    <div className={conversationStarted ? "w-full" : "mx-auto max-w-[1550px]"} dir="rtl">
      {!conversationStarted && (
        <section className="mb-6 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Button onClick={() => createSession.mutate({ title: "محادثة جديدة" })} disabled={createSession.isPending} className="h-11 rounded-xl bg-[#24332b] px-5 text-white shadow-lg shadow-[#24332b]/10 hover:bg-[#17221c]">
      ...
      <div className={conversationStarted ? "w-full" : "grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]"}>
        <section className={conversationStarted ? "flex min-h-[calc(100vh-2rem)] flex-col overflow-hidden bg-background" : "workspace-card min-h-[680px] overflow-hidden"}>
            ...
            <div className="flex flex-wrap gap-2 border-b border-[#ebe8e0] bg-[#fcfbf8] px-5 py-3 sm:px-6">
              {pendingFiles.map(file => (
                <span key={file.tempId} className="flex max-w-full items-center gap-2 rounded-xl border border-[#e3d8ce] bg-[#fffbf7] px-3 py-2 text-xs text-[#56635a]">
                  <Loader2 className="size-3.5 shrink-0 animate-spin text-[#bd653e]" /><span className="max-w-32 truncate font-semibold">{file.fileName}</span><span className="rounded-md bg-[#f4eadf] px-1.5 py-0.5 text-[9px] font-bold text-[#a95c3d]">{formatFileType(file.mimeType)}</span><span className="text-[#9aa19c]">{formatBytes(file.sizeBytes)}</span><span className="text-[10px] font-bold text-[#b66c48]">جارٍ الرفع</span>
                </span>
              ))}
              {attachedFiles.map(file => (
                <span key={file.id} className="flex max-w-full items-center gap-2 rounded-xl border border-[#dbe5dc] bg-white px-3 py-2 text-xs text-[#56635a]">
                  <FileText className="size-3.5 shrink-0 text-[#bd653e]" /><span className="max-w-32 truncate font-semibold">{file.fileName}</span><span className="rounded-md bg-[#edf4ee] px-1.5 py-0.5 text-[9px] font-bold text-[#397048]">{formatFileType(file.mimeType)}</span><span className="text-[#9aa19c]">{formatBytes(file.sizeBytes)}</span><span className="text-[10px] font-bold text-[#4b8058]">{file.status === "ready" ? "جاهز" : file.status === "processing" ? "قيد المعالجة" : "تعذّر التحضير"}</span><button onClick={() => setAttachedFiles(previous => previous.filter(item => item.id !== file.id))} className="rounded-md p-0.5 hover:bg-[#f0eee8]" aria-label={`إزالة ${file.fileName}`}><X className="size-3.5" /></button>
                </span>
              ))}
            </div>
          )}
          ...
          {voiceDraft && (
            <div className="m-4 flex flex-col gap-3 rounded-2xl border border-[#dce8de] bg-[#f4faf4] p-4 sm:m-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-start gap-3"><Mic className="mt-0.5 size-4 shrink-0 text-[#4a7657]" /><div><p className="text-xs font-bold text-[#365640]">مسودة من التسجيل الصوتي</p><p className="mt-1 line-clamp-2 text-xs leading-6 text-[#617069]">{voiceDraft}</p></div></div>
          ...
          <AIChatBox messages={chatMessages} onSendMessage={sendPrompt} isLoading={sendMessage.isPending || createSession.isPending} height={conversationStarted ? "calc(100vh - 9rem)" : "560px"} className="m-0 flex-1 rounded-none border-0 shadow-none" placeholder="اكتب طلبك بتفصيل… استخدم Shift + Enter لسطر جديد" emptyStateMessage="ابدأ بما تريد إنجازه، وسأساعدك في ترتيب العمل وتنفيذه." suggestedPrompts={["حلّل هذه الفكرة وحوّلها إلى خطة تنفيذ.", "اكتب مسودة احترافية بناءً على ملاحظاتي.", "راجع هذا المحتوى واقترح تحسينات عملية."]} />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#ebe8e0] bg-[#fcfbf8] px-5 py-3 sm:px-6">
            <div className="flex items-center gap-2">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={isRecording ? stopRecording : startRecording} disabled={uploadFile.isPending || transcribeVoice.isPending} className={`rounded-xl ${isRecording ? "bg-red-50 text-red-700 hover:bg-red-100" : "text-[#5d6b62] hover:bg-[#eceae3] hover:text-[#25372b]"}`}>
                {isRecording ? <Square className="ml-1.5 size-3.5 fill-current" /> : <Mic className="ml-1.5 size-4" />}{isRecording ? "إيقاف التسجيل" : "رسالة صوتية"}
              </Button>
            </div>
        ...
        {!conversationStarted && (
          <aside className="flex flex-col gap-5">
            <section className="workspace-card overflow-hidden">
              <div className="flex items-center justify-between border-b border-[#ebe8e0] px-5 py-4"><div><p className="text-sm font-extrabold text-[#26372c]">الجلسات</p><p className="mt-0.5 text-[11px] text-[#87908b]">سياق محفوظ لكل مهمة</p></div><span className="rounded-lg bg-[#f0eee8] px-2 py-1 text-[11px] font-bold text-[#718078]">{sessions.length}</span></div>
              <div className="max-h-[300px] space-y-1 overflow-y-auto p-2">
                {sessionsQuery.isLoading ? <div className="flex items-center gap-2 p-4 text-xs text-[#8b948f]"><Loader2 className="size-3.5 animate-spin" />جارٍ تحميل الجلسات…</div> : sessions.length === 0 ? <div className="p-4 text-center"><FolderOpen className="mx-auto size-6 text-[#c0c5bf]" /><p className="mt-2 text-xs leading-5 text-[#7d8781]">لا توجد جلسات بعد. ابدأ أول محادثة الآن.</p></div> : sessions.map(session => <button key={session.id} onClick={() => setActiveSessionId(session.id)} className={`w-full rounded-xl px-3 py-3 text-right transition-colors ${activeSessionId === session.id ? "bg-[#e7eee8]" : "hover:bg-[#f4f2ed]"}`}><p className="truncate text-xs font-bold text-[#2c3b31]">{session.title}</p><p className="mt-1 text-[10px] text-[#8c948f]">{new Date(session.updatedAt).toLocaleDateString("ar-SA", { month: "short", day: "numeric" })}</p></button>)}
              </div>
            </section>
            <section className="rounded-[1.4rem] bg-[#24332b] p-5 text-white shadow-[0_16px_35px_-20px_rgba(36,51,43,0.6)]"><p className="text-xs font-bold text-[#edc6ae]">سير عمل أسرع</p><h3 className="mt-2 text-base font-extrabold leading-7">ابدأ من الأدوات عندما يكون هدفك واضحًا.</h3><p className="mt-2 text-xs leading-6 text-[#c6d0c9]">اختر مسارًا للكتابة أو البرمجة أو التحليل، ثم تابع النتيجة داخل جلسة جديدة.</p><a href="/tools" className="mt-4 inline-flex rounded-xl bg-white/12 px-3 py-2 text-xs font-bold text-white transition-colors hover:bg-white/20">استعراض الأدوات</a></section>
            <section className="workspace-card p-5"><p className="text-xs font-extrabold text-[#34463a]">مرفقات هذه الجلسة</p>{sessionFilesQuery.isLoading ? <p className="mt-3 text-xs text-[#8b948f]">جارٍ التحميل…</p> : (sessionFilesQuery.data?.length ?? 0) === 0 ? <p className="mt-3 text-xs leading-6 text-[#89918c]">أرفق ملفًا ليظهر هنا ويصبح جزءًا من سياق عملك.</p> : <div className="mt-3 space-y-2">{sessionFilesQuery.data?.slice(0, 4).map(file => <a href={file.storageUrl} key={file.id} className="flex items-center gap-2 rounded-xl bg-[#f5f3ed] p-2.5 text-xs text-[#4f5e54] hover:bg-[#eceae4]"><FileText className="size-3.5 shrink-0 text-[#bd653e]" /><span className="min-w-0 flex-1"><span className="block truncate font-bold">{file.fileName}</span><span className="mt-1 flex items-center gap-1.5 text-[10px] text-[#8b948f]"><span>{formatFileType(file.mimeType)}</span><span>·</span><span>{file.status === "ready" ? "جاهز للاستخدام" : file.status === "processing" ? "قيد المعالجة" : "تعذّر التحضير"}</span></span></span></a>)}</div>}</section>
          </aside>
        )}
      </div>
    </div>
