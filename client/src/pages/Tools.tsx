...
import { categories as skillCategories, workflows as skillWorkflows, type Category, type Workflow } from "@/data/skillCatalog";
import { BarChart3, Braces, Check, FilePenLine, Github, ListChecks, Loader2, Rocket, Search, ShieldCheck, Sparkles, WandSparkles, GitBranch, Zap } from "lucide-react";
...
10. إذا كانت هناك معلومة ضرورية فعلًا، اسأل عنها قبل التنفيذ؛ وإلا استخدم افتراضًا منطقيًا واذكره.`;
  ...
  const filteredSkills = useMemo(() => skillWorkflows.filter(workflow => {
    ...
    <div className="mx-auto max-w-[1240px] pb-12" dir="rtl">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#17241d] px-6 py-8 text-white shadow-[0_30px_90px_-35px_rgba(18,40,28,.9)] sm:px-10 sm:py-11">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div><div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-bold text-[#bde8cc]"><WandSparkles className="size-3.5" /> AI APP FACTORY</div><h1 className="mt-4 text-3xl font-black tracking-tight sm:text-5xl">حوّل فكرتك إلى تطبيق.</h1><p className="mt-4 max-w-2xl text-sm leading-8 text-[#c9d4cd]">أدخل فكرتك وميزاتك، وسيتولى المسار تخطيط المنتج والواجهة والـAPI وقاعدة البيانات والاختبارات وتجهيز المشروع لـ GitHub والنشر.</p><div className="mt-6 flex flex-wrap gap-2 text-xs text-[#aebbb3]">{["واجهة مستقبلية", "Full-Stack", "Database", "API", "Tests", "GitHub Ready"].map(x => <span key={x} className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5">{x}</span>)}</div></div>
          <div className="rounded-3xl border border-white/10 bg-black/15 p-5 backdrop-blur-xl"><div className="flex items-center justify-between text-xs text-[#b9c7bf]"><span>جاهزية المشروع</span><span className="text-[#bde8cc]">LIVE BUILDER</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[88%] rounded-full bg-[#bde8cc]" /></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-[10px] text-[#aebbb3]"><div className="rounded-2xl border border-white/10 p-3"><div className="text-lg font-black text-white">01</div>الفكرة</div><div className="rounded-2xl border border-white/10 p-3"><div className="text-lg font-black text-white">02</div>البناء</div><div className="rounded-2xl border border-white/10 p-3"><div className="text-lg font-black text-white">03</div>GitHub</div></div></div>
        </div>
      </section>

      <section className="mt-7 overflow-hidden rounded-[2rem] border border-[#dfe7e1] bg-[#fbfcfa] shadow-[0_24px_70px_-45px_rgba(35,55,44,.35)]"><div className="border-b border-[#e5ebe6] px-6 py-5 sm:px-8"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-2xl bg-[#e4f4ea] text-[#2f7a50]"><Rocket className="size-5" /></span><div><h2 className="text-xl font-black text-[#203129]">مولّد التطبيقات الأضخم</h2><p className="text-xs text-[#77827c]">املأ المدخلات وسنحوّلها إلى مواصفات تنفيذية واضحة.</p></div></div></div><div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-2">
      ...
      <section className="mt-7 overflow-hidden rounded-[2rem] border border-[#dfe7e1] bg-white shadow-[0_24px_70px_-45px_rgba(35,55,44,.28)]"><div className="border-b border-[#e5ebe6] px-6 py-5 sm:px-8"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-2xl bg-[#edf4ef] text-[#3c7653]"><ListChecks className="size-5" /></span><div><h2 className="text-xl font-black text-[#203129]">خطة ترقية الأدوات الأضخم</h2><p className="text-xs text-[#77827c]">المسار المقترح للانتقال من مولّد أفكار إلى مصنع تطبيقات متكامل.</p></div></div></div><div className="grid gap-4 p-6 sm:p-8 md:grid-cols-2 lg:grid-cols-3">{upgradePlan.map(({ step, title, description, icon: Icon }) => <article key={step} className="group rounded-3xl border border-[#e1e8e3] bg-[#fbfcfa] p-5 transition-all hover:-translate-y-1 hover:border-[#b9d8c4] hover:shadow-[0_18px_40px_-30px_rgba(35,55,44,.5)]"><div className="flex items-center justify-between"><span className="text-xs font-black text-[#7a8b82]">PHASE {step}</span><span className="flex size-10 items-center justify-center rounded-2xl bg-[#e8f3eb] text-[#377751]"><Icon className="size-5" /></span></div><h3 className="mt-5 text-base font-black text-[#26372e]">{title}</h3><p className="mt-2 text-sm leading-7 text-[#707b74]">{description}</p></article>)}</div><div className="mx-6 mb-6 rounded-2xl border border-[#dce8df] bg-[#f4f8f5] p-4 sm:mx-8 sm:mb-8"><div className="flex items-start gap-3"><Check className="mt-0.5 size-4 text-[#39815a]" /><p className="text-xs leading-6 text-[#66736b]"><strong className="text-[#30473a]">الأولوية:</strong> نبدأ بتوليد الملفات فعليًا، ثم GitHub، ثم الاختبارات والنشر التلقائي. كل مرحلة تُبنى فوق السابقة بدون اختراع أسرار أو تجاوز صلاحيات المستخدم.</p></div></div></section>

      <section className="mt-7 rounded-[2rem] border border-[#dfe7e1] bg-[#fbfcfa] p-6 sm:p-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-bold tracking-[0.15em] text-[#bd653e]">مكتبات المهارات</p><h2 className="mt-2 text-2xl font-black text-[#203129]">كل مهارة لها سياقها وملفاتها ونتائجها.</h2><p className="mt-2 max-w-2xl text-sm leading-7 text-[#707b74]">افتح المكتبة المناسبة بدل بدء محادثة عامة، ثم احتفظ بكل عملك داخل سياق مستقل.</p></div><div className="relative w-full lg:max-w-xs"><Search className="pointer-events-none absolute right-3 top-1/2 size-4 -translate-y-1/2 text-[#89938c]" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="ابحث عن مكتبة..." className="h-11 rounded-xl border-[#dce5df] bg-white pr-10 text-right text-sm" /></div></div><div className="mt-5 flex gap-2 overflow-x-auto pb-1">{skillCategories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold transition-colors ${activeCategory === category ? "bg-[#24332b] text-white" : "bg-white text-[#6f7a71] hover:bg-[#e5ebe4]"}`}>{category}</button>)}</div><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filteredSkills.map(workflow => { const Icon = workflow.icon; return <article key={workflow.id} className="workspace-card flex min-h-[230px] flex-col p-5"><div className="flex items-start justify-between"><span className={`flex size-11 items-center justify-center rounded-2xl ${workflow.tone}`}><Icon className="size-5" /></span><span className="rounded-full bg-[#f7f5ef] px-2.5 py-1 text-[10px] font-bold text-[#8a928a]">{workflow.tag}</span></div><h3 className="mt-5 text-lg font-extrabold text-[#223229]">{workflow.title}</h3><p className="mt-2 flex-1 text-sm leading-7 text-[#6b766f]">{workflow.description}</p><Button onClick={() => openLibrary(workflow)} className="mt-5 h-10 rounded-xl bg-[#263a2e] text-white hover:bg-[#18291f]"><Sparkles className="ml-2 size-4" /> فتح مكتبة المهارة</Button></article>; })}</div></section>

      <section className="mt-7 grid gap-5 md:grid-cols-3">{factoryWorkflows.map(({ id, title, description, icon: Icon }) => { const libraryId = id === "analysis" ? "content-analysis" : id; const library = skillWorkflows.find(item => item.id === libraryId); return <article key={id} className="workspace-card flex min-h-[210px] flex-col p-6 transition-transform hover:-translate-y-1"><span className="flex size-11 items-center justify-center rounded-2xl bg-[#edf4ef] text-[#3c7653]"><Icon className="size-5" /></span><h3 className="mt-5 text-lg font-black text-[#26372e]">{title}</h3><p className="mt-2 flex-1 text-sm leading-7 text-[#707b74]">{description}</p><Button onClick={() => library && openLibrary(library)} className="mt-5 h-10 rounded-xl bg-[#263a2e] text-white hover:bg-[#18291f]"><Sparkles className="ml-2 size-4" /> فتح المكتبة</Button></article>; })}</section>
    </div>
  );
}
