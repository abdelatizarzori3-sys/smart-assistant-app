...
// Prefer the explicitly configured/non-Gemini production model first. Gemini remains
// available as a fallback, but an exhausted Gemini free-tier quota must not take down chat.
...
async function createFileAwarePrompt(input: { content: string; files: Awaited<ReturnType<typeof getWorkspaceFilesByIdsForUser>> }) {
  ...
  for (const file of input.files) {
    ...
    if (["application/pdf", "audio/mpeg", "audio/wav", "audio/mp4", "video/mp4"].includes(file.mimeType)) {
      promptParts.push({ type: "file_url", file_url: { url: signedUrl, mime_type: file.mimeType as "audio/mpeg" | "audio/wav" | "audio/mp4" | "video/mp4" } }); continue;
    }
...
export const appRouter = router({
  ...
  workspace: router({
    sessions: router({
      ...
      archive: protectedProcedure.input(z.object({ sessionId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { await requireOwnedSession(input.sessionId, ctx.user.id); await archiveWorkspaceSession(input.sessionId, ctx.user.id); return { success: true } as const; }),
    }),
    messages: router({
      list: protectedProcedure.input(z.object({ sessionId: z.number().int().positive() })).query(async ({ ctx, input }) => { await requireOwnedSession(input.sessionId, ctx.user.id); return listWorkspaceMessages(input.sessionId); }),
      send: protectedProcedure.input(z.object({ sessionId: z.number().int().positive(), content: z.string().trim().min(1).max(12000), fileIds: z.array(z.number().int().positive()).max(6).default([]) })).mutation(async ({ ctx, input }) => {
        ...
        await createWorkspaceMessage({ sessionId: input.sessionId, role: "user", content: input.content });
        ...
        try {
          ...
          for (let round = 0; githubEnabled && round < 3; round++) {
            ...
            for (const call of toolCalls) {
              ...
              try { const result = await executeWorkspaceTool(call, ctx.user.id); toolResults.push(JSON.stringify(result)); }
              catch (error) { toolResults.push(JSON.stringify({ error: error instanceof Error ? error.message : String(error) })); }
            }
            baseMessages.push({ role: "assistant", content: completion.choices[0]?.message.content || "", ...(toolCalls.length ? { tool_call_id: toolCalls[0].id } : {}) });
            for (let i = 0; i < toolResults.length; i++) baseMessages.push({ role: "tool", tool_call_id: toolCalls[i]?.id, content: toolResults[i] || "{}" });
            completion = await invokeLLM({ model, maxTokens: 1800, messages: baseMessages, tools: GITHUB_WORKSPACE_TOOLS, toolChoice: "auto" });
          }
          const assistantText = extractAssistantText(completion);
          if (!assistantText.trim()) throw new Error("لم يُرجع نموذج الذكاء الاصطناعي نصًا صالحًا.");
          const assistantMessage = await createWorkspaceMessage({ sessionId: input.sessionId, role: "assistant", content: assistantText });
          await createWorkspaceResult({ userId: ctx.user.id, sessionId: input.sessionId, title: createSessionTitle(input.content), content: assistantText, resultType: "assistant" });
          return { message: assistantMessage, model, capabilityId };
        } catch (error) {
          console.error("[workspace.messages.send] LLM failure", error);
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: error instanceof Error ? error.message : "تعذّر توليد الرد حاليًا." });
        }
      }),
    }),
    files: router({
      list: protectedProcedure.input(z.object({ sessionId: z.number().int().positive().optional() }).optional()).query(({ ctx, input }) => input?.sessionId ? listWorkspaceFilesForSession(input.sessionId, ctx.user.id) : listWorkspaceFilesForUser(ctx.user.id)),
      upload: protectedProcedure.input(z.object({ fileName: z.string().trim().min(1).max(255), mimeType: z.string().trim().min(1).max(150), data: z.string().min(1), sessionId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => {
        const raw = input.data.includes(",") ? input.data.slice(input.data.indexOf(",") + 1) : input.data;
        const bytes = Buffer.from(raw, "base64");
        if (bytes.byteLength > MAX_FILE_BYTES) throw inputError("حجم الملف يتجاوز 16MB.");
        const storageKey = `workspace/${ctx.user.id}/${Date.now()}-${sanitizeFileName(input.fileName)}`;
        await storagePut(storageKey, bytes, input.mimeType);
        const file = await createWorkspaceFile({ userId: ctx.user.id, fileName: input.fileName, mimeType: input.mimeType, sizeBytes: bytes.byteLength, storageKey });
        if (input.sessionId) await attachWorkspaceFilesToSession([file.id], ctx.user.id, input.sessionId);
        return file;
      }),
    }),
    admin: router({
      overview: adminProcedure.query(({ ctx }) => getWorkspaceAdminOverview(ctx.user.id)),
      sessions: adminProcedure.query(({ ctx }) => listWorkspaceSessions(ctx.user.id)),
      archive: adminProcedure.input(z.object({ sessionId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { await archiveWorkspaceSessionAsAdmin(input.sessionId, ctx.user.id); return { success: true } as const; }),
    }),
  }),
});
