...
تمت إضافة `railway.json` إلى جذر المشروع ليثبت إعدادات البناء والتشغيل دون تغيير سلوك التطبيق الحالي. يستخدم Railway ‏Railpack، ثم ينفذ `pnpm build`، ويشغّل الخادم عبر `pnpm run start`. التطبيق يقرأ `PORT` الذي يحقنه Railway تلقائيًا، ويستخدم المسار `/health` لفحص الصحة.
